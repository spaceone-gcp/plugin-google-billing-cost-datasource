...
class CreditsDetailManager(BaseManager):
    ...
    def _create_credits_detail_sql(
        self,
        start_date: str,
        end_date: str = None,
        project_id: str = None,
        ...
        try:
            ...
            if credits_detail_raw and credits_detail_raw != "[]":
                ...
                try:
                    ...
                    _LOGGER.warning(
                        f"[CreditsDetailManager] Credits Detail JSON 파싱 실패: {credits_detail_raw}"
                    )
                    credits_detail = []

            # 결과 레코드 구성
            record = {
                "billed_date": row.get("billed_at", "").strftime("%Y-%m-%d")
                ...
                "currency_conversion_rate": float(
                    row.get("currency_conversion_rate", 1.0)
                ),
    ...
    def get_credits_summary(
        self, start_date: str, end_date: str = None, project_id: str = None
    ) -> Dict[str, Any]:
        """Credits 요약 정보 조회"""
        _LOGGER.info(
            f"[CreditsDetailManager] Credits 요약 조회 - {start_date} ~ {end_date or start_date}"
        )
        ...
        try:
            ...
            summary = {
                "total_records_with_credits": int(
                    row.get("total_records_with_credits", 0)
                ),
    ...
    def _create_credits_summary_sql(
        self, start_date: str, end_date: str = None, project_id: str = None
    ) -> str:
        """Credits 요약 조회용 SQL"""
