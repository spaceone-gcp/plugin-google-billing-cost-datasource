...
class DecimalJSONEncoder(json.JSONEncoder):
    ...
    def _convert_float_to_safe_number(self, value: float) -> float:
        ...
        try:
            ...
            if "e" in str(result).lower():
                _LOGGER.warning(
                    f"[DecimalJSONEncoder] Scientific notation detected, preserving original: {value}"
                )
            ...
            _LOGGER.warning(
                f"[DecimalJSONEncoder] Float conversion failed for {value}: {e}"
            )
            return value  # 원본 값 보존
    ...
    def _convert_decimal_to_safe_number(self, value: Decimal) -> float:
        ...
        try:
            ...
            if "e" in str(result).lower():
                _LOGGER.warning(
                    f"[DecimalJSONEncoder] Scientific notation in Decimal, preserving original: {value}"
                )
            ...
            _LOGGER.warning(
                f"[DecimalJSONEncoder] Decimal conversion failed for {value}: {e}"
            )
            ...
            for record in result["results"]:
                if isinstance(record, dict):
                    ...
                    if "cost" not in record:
                        record["cost"] = None
                        _LOGGER.warning(
                            "[DecimalJSONEncoder] Cost field missing, set to None"
                        )
                    elif record["cost"] is None:
                        _LOGGER.debug(
                            "[DecimalJSONEncoder] Cost field is None, preserving None"
                        )
